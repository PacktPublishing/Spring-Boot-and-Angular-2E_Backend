...
check_docker_compose() {
    ...
    # Support both legacy docker-compose and Docker CLI plugin (docker compose)
    ...
    print_warning "Docker Compose is not installed (not required for image build/push in this script)"
}
...
build_images() {
    ...
    for service in "${SERVICES[@]}"; do
        ...
        containerfile_path="$PROJECT_ROOT/$service_dir/Containerfile"
        dockerfile_path="$PROJECT_ROOT/$service_dir/Dockerfile"
        if [ -f "$containerfile_path" ]; then
            build_file="$containerfile_path"
        elif [ -f "$dockerfile_path" ]; then
            build_file="$dockerfile_path"
        else
            print_error "No Containerfile or Dockerfile found for $service in $service_dir"
            exit 1
        fi
        
        print_info "Building $service from $(basename "$build_file")..."
        
        if docker build -t "$service:latest" -f "$build_file" "$PROJECT_ROOT/$service_dir"; then
