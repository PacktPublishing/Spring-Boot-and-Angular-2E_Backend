 ...
 * Keeps error responses consistent and maps exceptions to proper HTTP status
 * codes.
 */
...
public class GlobalExceptionHandler {
        ...
        ResponseEntity<ApiError> unprocessable(DomainRuleViolationException ex, HttpServletRequest req) {
                return ResponseEntity.status(422).body(
                                ApiError.unprocessableEntity(ex.getMessage(), req.getRequestURI())); // Changed this
                                                                                                     // line
        }
        ...
        ResponseEntity<ApiError> lazyInitialization(LazyInitializationException ex, HttpServletRequest req) {
                return ResponseEntity.status(500).body(
                                ApiError.dataLoadingError("Failed to load related data. Please try again.",
                                                req.getRequestURI()));
        }
        ...
        ResponseEntity<ApiError> generic(Exception ex, HttpServletRequest req) {
                ...
                ex.printStackTrace();
                return ResponseEntity.status(500).body(
                                ApiError.internalServerError("An unexpected error occurred: " + ex.getMessage(),
                                                req.getRequestURI()));
        }
